...
var vm = new Vue({
    ...
    methods: {
        ...
        search:function(event){
            ...
            $.get("/api/customer/goodsOrders",model.searchObj,function(response){
        ...
        editInfo:function(){
            ...
            layer.confirm('请确认发货?', {icon: 3, title:'确认'}, function(index){
                //do something
                var editInfo = $("#editForm").serializeObject(); //初始化 model.search 对象
                $.get("/api/customer/handleOrder",editInfo,function(response){
                    if (response.success_is_ok){
                        parent.layer.closeAll();
                        layer.msg(response.msg,{icon: 6});
                        //刷新页面
                        window.location.href = "/customer/goodsorder";
                    }else{
