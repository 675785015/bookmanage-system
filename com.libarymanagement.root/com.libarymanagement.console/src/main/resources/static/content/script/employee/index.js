...
var vm = new Vue({
    ...
    methods: {
        ...
        download : function(){
            var strwhere="";
            if(model.searchObj.name!=null && model.searchObj.name!= "")
            {
                strwhere+="&name="+model.searchObj.name;
            }
            if(model.searchObj.mobile!=null && model.searchObj.mobile!= "")
            {
                strwhere+="&mobile="+model.searchObj.mobile;
            }
            if(model.searchObj.teamId!=null && model.searchObj.teamId!= "0")
            {
                strwhere+="&teamId="+model.searchObj.teamId;
            }
            if(model.searchObj.rankId!=null && model.searchObj.rankId!= "0")
            {
                strwhere+="&rankId="+model.searchObj.rankId;
            }

            window.open("/api/employee/download?"+strwhere.substr(1,strwhere.length));
        },
        //绑定表单验证
            ...
            vm.openFormVm = new Vue({
                ...
                computed:{
                    ...
                    partOfTeams:function(){
                        ...
                        var job=this.formData.rankId;
                        var localPartOfTeams = new Array();
                        if(departmentId>0 && job>1 ){


                            $.ajax({
                                type : "post",
                                url : "/api/employee/getByJob",
                                data : "job=" + job+"&store="+departmentId,
                                async : false,
                ...
                beforeCreate:function(){
                    ...
                    if (empId>0){
                        for (var i=0;i<vm.employees.length;i++){
                            ...
                            if (item.empId==empId){
                                openVmModel.formData.name = item.empName;
                                openVmModel.formData.mobile = item.empMobilephone;
                                openVmModel.formData.rankId = item.empJob;
                                openVmModel.formData.departmentId = item.empStore;
                                openVmModel.formData.teamId = item.empHigherId || 0;
                            }
                        }
                    }
                },
